...
opacity() {
    if [[ $(< "$HOME/.config/kitty/opacity.conf") != "background_opacity 0.90" ]]
    then
        for kitty in /tmp/kitty-*
        do
        kitten @ --to unix:$kitty set-background-opacity 0.90
        done
        echo "background_opacity 0.90" > "$HOME/.config/kitty/opacity.conf"
